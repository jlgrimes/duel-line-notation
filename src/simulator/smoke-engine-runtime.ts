...
export class SmokeEngineRuntime {
  ...
  private async execute(command: EngineCommand, events: EngineEvent[]): Promise<void> {
    if (command.type === "initialize") {
      await this.initialize(events);
      return;
    }

    if (command.type === "process-step") {
      this.processStep(command.state, events);
      return;
    }
  ...
  private async initialize(events: EngineEvent[]): Promise<void> {
    ...
    events.push(
      { type: "status", phase: "starting", message: this.currentSnapshot.statusMessage },
      {
        type: "log",
        level: "info",
        message: "Web Worker started",
        detail: "The simulator engine is isolated from the React render thread.",
      },
    );

    if (typeof WebAssembly === "undefined") {
      throw new Error("This runtime does not expose WebAssembly.");
    }

    events.push({
      type: "log",
      level: "info",
      message: "WebAssembly support detected",
      detail: "Instantiating the bridge smoke module.",
    });
    ...
    this.currentSnapshot = {
      phase: "ready",
      statusMessage: "WASM bridge ready",
      engineVersion,
      stepValue: 0,
    };
    events.push(
      {
        type: "log",
        level: "success",
        message: "WebAssembly bridge ready",
        detail: `ABI smoke version ${engineVersion} loaded from ${SIMULATOR_WASM_SMOKE_BYTES.byteLength} bytes.`,
      },
      {
        type: "log",
        level: "info",
        message: "Typed engine adapter ready",
        detail: "React now consumes snapshots and domain events instead of raw Worker or WebAssembly messages.",
      },
  ...
  private reset(events: EngineEvent[]): void {
    ...
    events.push(
      { type: "status", phase: "idle", message: this.currentSnapshot.statusMessage },
      {
        ...
        detail: "The worker remains alive and can initialize a fresh module instance.",
      },
    );
  }
}
